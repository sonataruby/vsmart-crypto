var ScrollingBackground  = function (scene, key, velocityY) {
    this.scene = scene;
    this.key = key;
...
ScrollingBackground.prototype.createLayers = function () {
    for (let i = 0; i < 2; i += 1) {
      const layer = this.scene.add.sprite(0, 0, this.key);
      layer.y = (layer.displayHeight * i);
      const flipX = irandom_range(0, 10) >= 5 ? -1 : 1;
      const flipY = irandom_range(0, 10) >= 5 ? -1 : 1;
